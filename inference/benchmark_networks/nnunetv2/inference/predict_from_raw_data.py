...
class nnUNetPredictor(object):
    ...
    def _internal_predict_sliding_window_return_logits(self,
        ...
        logits_nbytes = int(np.prod(logits_shape)) * torch.empty((), dtype=torch.half).element_size()
        memmap_threshold = int(os.environ.get("NNUNET_LOGITS_MEMMAP_THRESHOLD_BYTES", 1024 ** 3))
        logits_memmap = None
        if results_device.type == "cpu" and logits_nbytes >= memmap_threshold:
            memmap_dir = os.environ.get("NNUNET_LOGITS_MEMMAP_DIR", tempfile.gettempdir())
            os.makedirs(memmap_dir, exist_ok=True)
            fd, memmap_path = tempfile.mkstemp(prefix="nnunet-logits-", suffix=".fp16", dir=memmap_dir)
            os.close(fd)
            logits_memmap = np.memmap(memmap_path, mode="w+", dtype=np.float16, shape=logits_shape)
            predicted_logits = torch.from_numpy(logits_memmap)
            predicted_logits._nnunet_memmap_path = memmap_path
            print(
                f"Storing {logits_nbytes / 1024 ** 3:.2f} GiB nnU-Net logits in {memmap_path}",
                flush=True,
            )
        else:
            predicted_logits = torch.zeros(logits_shape, dtype=torch.half, device=results_device)
        ...
        # Work channel-wise so normalization and the finite-value check do not create
        # another whole-volume temporary allocation when logits are disk-backed.
        has_inf = False
        for channel in range(predicted_logits.shape[0]):
            predicted_logits[channel].div_(n_predictions)
            if torch.any(torch.isinf(predicted_logits[channel])):
                has_inf = True
                break
        if has_inf:
    ...
    def predict_sliding_window_return_logits(self, input_image: torch.Tensor) \
        ...
        with torch.no_grad():
            with torch.autocast(self.device.type, enabled=True) if self.device.type == 'cuda' else dummy_context():
                ...
                empty_cache(self.device)
                # revert padding
                memmap_path = getattr(predicted_logits, "_nnunet_memmap_path", None)
