...
class PatchMerging2D(nn.Module):
    def __init__(self, dim, out_dim=-1, norm_layer=nn.LayerNorm, channel_first=False):
        ...
        self.reduction = Linear(4 * dim, (2 * dim) if out_dim < 0 else out_dim, bias=False) # Linear (input_dim, output_dim) quindi viene dimezzato di default
        self.norm = norm_layer(4 * dim)

    @staticmethod
    def _patch_merging_pad_channel_last(x: torch.Tensor):
        H, W, _ = x.shape[-3:]
        # padding if the dimension is odd, to ensure both height and width are even numbers
        if (W % 2 != 0) or (H % 2 != 0):
...
class SS2Dv0:
    ...
    def forwardv0(self, x: torch.Tensor, SelectiveScan=SelectiveScanMamba, seq=False, force_fp32=True, **kwargs):
        x = self.in_proj(x) # d --> d*2
        x, z = x.chunk(2, dim=-1)  # (b, h, w, d) perchè creati due chunk lungo d
        z = self.act(z)
        ...
        # x.view() flattenize the tensor (B, D, H, W --> B, D*H*W), poi B, 2, D*H*W copying the dimension D*H*W
        # In the end view : B, 2, D*H*W --> B, 2, D, H*W(L)
        x_hwwh = torch.stack([x.view(B, -1, L), torch.transpose(x, dim0=2, dim1=3).contiguous().view(B, -1, L)],
                             dim=1).view(B, 2, -1, L)
        # Flip along the last dimension (the spatial one H*W) and concatenate along dim 1 (B, 2*2, D, L)
...
#Class to choose whether to use SS2Dv3, SS2Dv2 or SS2Dv0
