...
LOW_MEMORY_GPU_LIMIT_BYTES = 17 * 1024**3
LOW_MEMORY_PATCH_SIZE = (64, 160, 160)


def configure_inference_patch(configuration_manager, device: torch.device) -> None:
    """Reduce the sliding-window patch on 16 GB GPUs to cap cuDNN workspace use."""
    requested = os.environ.get("TOOTHFAIRY_PATCH_SIZE")
    if requested:
        try:
            patch_size = tuple(
                int(value) for value in requested.replace("x", ",").split(",")
            )
        except ValueError as error:
            raise ValueError(
                "TOOTHFAIRY_PATCH_SIZE must contain three integers, for example 64,160,160"
            ) from error
        if len(patch_size) != 3 or any(value <= 0 for value in patch_size):
            raise ValueError(
                "TOOTHFAIRY_PATCH_SIZE must contain three positive integers"
            )
        reason = "TOOTHFAIRY_PATCH_SIZE"
    elif (
        device.type == "cuda"
        and torch.cuda.get_device_properties(device).total_memory
        <= LOW_MEMORY_GPU_LIMIT_BYTES
    ):
        total_memory = torch.cuda.get_device_properties(device).total_memory
        patch_size = LOW_MEMORY_PATCH_SIZE
        reason = f"{total_memory / 1024**3:.2f} GiB GPU"
    else:
        return

    trained_patch = tuple(int(value) for value in configuration_manager.patch_size)
    patch_size = tuple(
        min(current, trained) for current, trained in zip(patch_size, trained_patch)
    )
    cumulative_stride = tuple(
        int(value)
        for value in np.prod(configuration_manager.pool_op_kernel_sizes, axis=0)
    )
    if any(value % stride for value, stride in zip(patch_size, cumulative_stride)):
        raise ValueError(
            f"Inference patch {patch_size} must be divisible by network stride {cumulative_stride}"
        )
    configuration_manager.configuration["patch_size"] = list(patch_size)
    print(
        f"Using memory-efficient nnU-Net patch {patch_size} ({reason}; trained patch {trained_patch})",
        flush=True,
    )
...
def load_predictor(
    ...
    configure_inference_patch(configuration_manager, device)
...
def predict_segmentation(predictor, case: PreprocessedCase) -> np.ndarray:
    ...
    memmap_path = getattr(logits, "_nnunet_memmap_path", None)
    try:
